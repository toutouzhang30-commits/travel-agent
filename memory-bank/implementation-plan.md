...
7. **PDF 只作为受控 RAG 知识源补充**：允许把北京 / 上海 / 杭州攻略 PDF 经抽取、清洗、标注后转为 `RagRawDocument` 并复用 `RagDocumentConverter` 入库；不做 PDF 导出，不直接把 PDF 原文塞进 prompt。
...
### 3.2 未完成但容易误判为已完成的能力
- Working Memory 仍偏薄：已有内存态窗口，但缺少 stableProfile、recentDecisions、目的地切换清理和工具证据时效驱逐。
- Reflection 仍未完整进入主链路：itinerary 生成已部分接入，知识问答和 itinerary 修改仍需补齐。
- 工具调用链路尚未形成受控 ReAct / Tool-use，目前仍以单次工具调用和失败降级为主。
...
| Phase 5 | Agent 编排 + Working Memory + Reflection + Route Validation | 部分接入：Working Memory 雏形与 itinerary Reflection 已进入；知识问答 / 修改 Reflection、工具受控 ReAct 和路线校验待补齐 |
...
- `content_hash` 基于所有受控原始知识文档计算，包括 `classpath:/rag/*.md` 以及后续 PDF loader 输出的 `RagRawDocument` 文件名和内容。
...
### 后续补充：受控 PDF 攻略入库
PDF 入库不恢复旧的“PDF 目标”，而是作为 RAG 知识源扩展纳入现有 manifest 入库链路。
...
实施要求：
- 新增 `PdfRagDocumentLoader`，归属 `chat.rag.ingest.component`。
- PDF loader 只负责读取 PDF、清理页眉页脚 / 页码 / 目录 / 广告 / 重复导航，并包装成现有 `--- metadata --- content` 格式。
- PDF loader 不直接产出 Spring AI `Document`，必须复用 `RagDocumentConverter`，保持 chunk 策略、metadata 结构、manifest hash 和 active run 机制一致。
- PDF 原始路径通过配置控制，第一批只接入北京 / 上海 / 杭州三个攻略 PDF。
- PDF metadata 至少包含 `city`、`category`、`source`、`sourceType: "PDF"`、`verifiedAt`、`confidenceLevel`。
- PDF 中涉及实时天气、实时票价、实时路线耗时的内容不得作为静态知识确定回答，仍由 Tool 或 RAG gate 边界处理。
...
当前路线调整为先跑通完整候选治理链路，不先做 RRF、score fusion 和系统性评估调参。RAG 质量治理必须明确为“规则校验 + 按需 LLM-as-Judge + 少量人工抽检”三层，不再依赖人工逐条查看检索结果。当前主链路为：
...
其中 BM25 负责补召回，DashScope Reranker 负责排序，`RagRetrievalGate` 负责规则层拦截，`RagEvidenceJudge` 负责在低置信、复杂问题、候选冲突或评估场景按需判断“证据是否足以回答”。人工抽检用于校准评估集、阈值和 judge prompt，不作为在线回答必经链路。RRF、score fusion、系统性分数评估和指标调优放到这条链路稳定之后。

### 关键能力
- 数据安全：评估测试不能触发入库，入库失败不能清空旧 active run，manifest 必须和向量表真实数据一致。
- 评估基准：正样本、负样本、拒答指标和报告输出必须先建立。
- 人工抽检：只抽查评估集、线上低置信样例和 judge 分歧样例，用于校准阈值、prompt 和评估数据，不参与每次在线回答。
...
### Phase 3B-7：按需 RagEvidenceJudge + 上下文压缩与来源治理
在 reranker 之后新增 `RagEvidenceJudge`。它是证据充分性判断层，不替代 reranker。

要求：
- `RagEvidenceJudge` 是子代理式裁判，但对外必须封装为受控 Spring Service。
- 第一版采用按需在线策略：只在规则层低置信、复杂问题、候选冲突或评估场景触发，不要求每次 RAG 回答都调用。
- 规则层已经明确拒绝的实时问题、错城市、无来源、topK 为空等情况，不再调用 judge 消耗模型成本。
- judge 只判断证据是否足够回答，不直接生成最终答案。
- judge 必须输出结构化 DTO，例如 `RagEvaluationResult`，包含 `evidenceEnough`、`sourceActuallyAnswersQuestion`、`relevanceScore`、`coverageScore`、`groundingScore`、`missingFacts`、`freshnessRequired`、`reason`、`fallback`。
- 不暴露隐藏推理过程。
...
- 每条样例输出 `gateAllowed`、`gateReason`、`bestScore`、`lexicalScore`、`rerankScore`、`rerankTimeMs`、`judgeTriggered`、`judgeDecision`、`manualReviewRequired`、`fallback`、实际 topK、各阶段分数和失败原因。
- 少量人工抽检只标记样例和校准结论，不替代 gate 或 judge 的在线决策。
...
- `RagRetrievalGate` 先完成规则层拦截，`RagEvidenceJudge` 只在按需场景使用批处理策略判断证据充分性，并且只允许通过批次进入上下文。
- 评估报告能记录规则校验结果、judge 触发原因、judge 决策和人工抽检标记。
- 过旧或低可信来源不会优先进入最终上下文。
- 实时天气、票价、路线类问题不会被 RAG 当成静态知识确定回答。
- 低相关、错城市、无来源或负样本命中的结果会被 `RagRetrievalGate` 拒绝。
- RAG 评估报告能给出 Recall、MRR、负样本误召回率、拒答指标、延迟数据、vector score、lexical score、RRF score、rerank score、最终 gate reason、judge 结果和人工抽检标记。
...
当前 Phase 5 的分工已经明确：
- 知识问答、itinerary 生成和 itinerary 修改更适合 Reflection，用于检查证据充分性、结果是否满足约束、修改是否真正落实，并最多触发一次局部修正。
- 工具调用链路更适合受控 ReAct / Tool-use，用于参数提取、工具调用、观察结果、一次补救或降级回答。
- itinerary 最终确认需要新增路线校验闭环：MapsTool 提供客观路线、距离、耗时和交通方式数据，`RouteValidationAgent` 判断路线合理性、节奏压力和绕路风险。
- 受控 ReAct 不是开放式无限 Agent；每次工具调用最多允许一次补救动作，例如补参数、换默认交通方式、重试一次或明确失败。

### 当前接入状态
- 已有内存态 Working Memory：保存当前 itinerary 摘要、最近对话、最近工具证据和最近 Reflection 摘要。
- 已有 itinerary 生成链路 Reflection：生成草稿后结合天气 / 地图证据检查，并最多触发一次 `ItineraryModifier` 局部修正。
- `RouteValidationAgent` 尚未实现；当前只有 MapsTool 第一版可提供单次路线查询数据，尚未形成 itinerary 级路线校验。
- 仍未完成：stableProfile、recentDecisions、目的地切换清理、工具证据时效驱逐、知识问答 Reflection、itinerary 修改 Reflection、工具受控 ReAct / Tool-use、路线校验闭环。
...
- 检查 itinerary 的路线顺序、相邻景点交通耗时、每日通勤压力和绕路风险。
- 检查用户修改请求是否真实落到新 itinerary。
- 知识问答 Reflection 重点检查证据是否足够、是否过度确定、是否需要不确定性表达。
- itinerary 修改 Reflection 重点检查修改请求是否被局部落实，不能无故整份重写。
- 工具链路只做轻量输出校验：工具失败是否被明确提示、是否误把失败伪装成成功、是否带来源和更新时间。

### Route Validation 要求
- `RouteValidationAgent` 未来归属 `chat.agent.route`，不放进 `chat.tool.maps`；它判断行程合理性，MapsTool 只提供客观路线证据。
- `MapsTool` 继续归属 `chat.tool.maps`，输出路线、距离、耗时、交通方式、来源和更新时间。
- `RouteValidationResult` 目标字段包括 `feasibilityScore`、`paceScore`、`routeEfficiencyScore`、`issues`、`suggestions`、`needsRevision`。
- `RouteValidationAgent` 应在 itinerary 生成和 itinerary 修改后运行，重点识别跨区绕路、通勤过长、一天过满、路线证据缺失。
- 若 `needsRevision=true`，最多触发一次 `ItineraryModifier` 局部修正；不允许无限循环或无故整份重写。
- MapsTool 失败时，校验结果必须明确“缺少实时路线依据”，不能伪造耗时或把静态攻略当作路线结论。

### 工具调用受控 ReAct / Tool-use 要求
- 流程固定为：结构化参数提取 -> 工具调用 -> 观察工具结果 -> 最多一次补救或降级 -> 最终回答。
- 补救动作必须有上限，默认最多一次，不允许无限循环。
- 参数缺失时优先使用 Working Memory 补全；仍缺失时追问。
- 工具失败时可以按工具类型执行一次安全补救，例如地图超时重试一次、路线参数不清时追问、天气城市缺失时用当前目的地补全。
- 最终回答必须明确 success / failure、来源、更新时间和不确定性。

### 完成标准
- 多轮对话不再只依赖当前一句 message。
- “第二天太累了，轻松一点”能结合已有 itinerary 局部修改。
- 生成杭州 3 天 itinerary 后，可以用 MapsTool 获取相邻景点路线耗时，并由 `RouteValidationAgent` 判断是否绕路或过满。
- itinerary 修改后需要再次检查路线和节奏，尤其是用户要求“轻松一点”时。
- 工具失败时，最终回答不会假装拿到实时数据。
- 预算/节奏明显不合理时，Reflection 能提示风险或做一次局部修正。
- 路线校验发现明显问题时，只允许一次局部修正。
- 直接工具问题可以完成一次受控 ReAct：参数提取、调用、观察、一次补救或降级、最终回答。
...
### 关键能力
- 最小规则层：预算档位、下雨替代方案、节奏过满提醒。
- 路线合理性主链路前移到 Phase 5 的 `RouteValidationAgent`；Phase 8 只负责路线校验指标、告警和上线前质量监控。
...
3. **Working Memory 继续补齐**：已有内存态雏形，下一步补 stableProfile、recentDecisions、目的地切换清理和工具结果时效驱逐。
4. **Reflection、Route Validation 与受控 ReAct 分工推进**：知识问答 / itinerary 生成修改补 Reflection；itinerary 最终确认补 `RouteValidationAgent`；工具调用补受控 ReAct / Tool-use。
...
- Working Memory 已经开始保存短期上下文，但还不足以稳定支撑路由、工具参数补全、修改和 Reflection。
- itinerary 生成已有 Reflection 雏形，但知识问答和 itinerary 修改仍需要反思闭环；路线可执行性需要基于 MapsTool 证据新增 `RouteValidationAgent`；工具调用则更适合受控 ReAct。
...
## 8. 当前明确不做
- 不做 PDF 导出。
- 不把未经清洗和标注的 PDF 原文直接入库或直接塞进 Prompt。
- 不继续扩大关键词/限制词列表。
- 不把 MCP 写成当前必须落地。
...
3. 实时问题优先走工具层，并通过受控 ReAct / Tool-use 处理参数、失败观察、一次补救和降级，不再伪装成 RAG 答案。
4. RAG 支持网页采集治理、多路召回、重排和来源展示。
5. itinerary 可以持续生成、修改，并保留来源与不确定性边界。
6. Reflection 能对知识问答、itinerary 生成和 itinerary 修改中的明显问题做一次有界局部修正。
7. RouteValidationAgent 能结合 MapsTool 路线证据发现绕路、过满或缺少实时路线依据的问题，并最多触发一次局部修正。
8. SSE 可以展示用户可见的执行过程。

---

## 10. 一句话执行策略
当前项目已经完成 Phase 4A 结构化路由前置、RAG 最小闭环、WeatherTool 受控工具展示和基于高德 Amap 的 MapsTool 第一版调用闭环；Phase 5 已部分接入内存态 Working Memory 和 itinerary 生成 Reflection。PricingTool 暂不做真实调用，当前继续推进 RAG 三层质量治理收口，并补齐知识问答 / itinerary 修改 Reflection、基于 MapsTool 证据的 RouteValidationAgent 路线校验闭环与工具调用受控 ReAct / Tool-use。
